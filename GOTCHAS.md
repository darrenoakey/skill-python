...
- `Path(__file__).parent.resolve()` in a `run` facade breaks when invoked through a symlink (e.g. the standard `~/bin/<name>` wrapper pattern, or when `~/bin` itself is symlinked into iCloud Drive/Dropbox). `.parent` is taken on the **unresolved** `__file__` (the symlink path) first, so `.resolve()` only resolves the parent directory's own symlinks — it never follows the final-component symlink (the script itself) to its real target, leaving `SCRIPT_DIR` pointed at the symlink's containing directory instead of the real project directory. Fix: resolve `__file__` itself first — `Path(__file__).resolve().parent`. Symptom: `run` (or any symlink-invoked script) fails to find `src/main.py` / `.venv` with a path that looks like the symlink's directory plus the real project's subpath glued together.
- `Path.symlink_to(target.relative_to(X))` rarely does what you think. A symlink's relative target is resolved from the directory the **link lives in**, not from `X`. If the link is at `outdir/final/foo.mp4` and you call `symlink_to(Path('foo/foo.mp4'))` (computed via `.relative_to(outdir)`), following the link looks for `outdir/final/foo/foo.mp4` — not `outdir/foo/foo.mp4`. Correct form: `symlink_to(Path(os.path.relpath(target, link.parent)))` so the target always has the right number of `..` segments. Even better for pipelines whose return value gets consumed by hash-based file registries: return the absolute real path, not the symlink — symlinks can be broken by a future `rm -rf output/final` or a sibling-dir rename and the consumer will never know until it tries to stat the file.
...
- `safetensors.safe_open(..., device="cuda")` is NOT a GPU mmap. It host-mmaps the file then does `torch.asarray(device="cuda")` per tensor, which cudaMemcpys bytes into a fresh CUDA allocation. On discrete-GPU boxes that's a PCIe transfer; on unified-memory boxes (Grace Blackwell / GB10) it's DRAM→DRAM but still a copy. There is no way to make torch CUDA tensors literally alias mmap'd host pages. "Switch to safetensors for zero-copy GPU loading" is a myth.
